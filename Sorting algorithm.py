...
# Quicksort
def quicksort(arr):#function which takes a single parameter, arr, which is the array to be sorted.
    if len(arr) <= 1:# this is the base case of the recursive function. If the array has only one element or is empty, it is already sorted
        return arr
    pivot = arr[len(arr) // 2]
...
def mergesort(arr):
    if len(arr) <= 1:
        return arr
    mid = len(arr) // 2#array is divided into two halve. The middle index is calculated
    left = mergesort(arr[:mid])#The left half of the array is sorted recursively
